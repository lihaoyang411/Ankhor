...
public class PlayerHandR : MonoBehaviour
{
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if (other.tag == "Flashlight")
        {
            ...
            if (!holdingObject)
            {
                ...
                // if you are on the other flashlight that your hand has
                // switch to it
                /*
                if (HandL.GetComponent<PlayerHandL>().holdingObject)
                {
                    OnStayPickupAction(other);

                    HandL.GetComponent<PlayerHandL>().holdingObject = false;

                    HandL.GetComponent<PlayerHandL>().ItemOnHand = null;
                }

                else
                {
                    // allow the player to pick it up cause 
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.tag == "Handle")
        {
            ...
            // hide the hand on the handle
            if (isSpirit)
            {
                handle_script.HideSpiritRightHand();

                // show the spirit hand

                GameObject.Find("RightHand").GetComponent<Valve.VR.InteractionSystem.Hand>().SetRenderModel(SpiritHandRender);
            }
            else
            {
                handle_script.HideSpiritRightHand();
    ...
    private void OnEnterHandleAction(Collider otherHandle)
    {
        ...
        if (isSpirit)
        {
            ...
            handle_script.ShowSpiritRightHand();
        }
        else
        {
            // hide your hand 
        ...
        if (PressedTrigger())
        {
            ...
            if (!door_script.isOpen)
            {
                ...
                bool openSuccessful = handle_script.Open_Close_Door(isSpirit);
    ...
    private void PickUpFlashlight(Collider otherflashlight)
    {
        ...
        if (PressedTrigger())
        {
            ...
            ItemOnHand.transform.parent = flashlight_script.gameObject.transform;
